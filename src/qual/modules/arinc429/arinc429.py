...
class ARINC429(Module):
    ...
    def updateChanStatus(self, response, inputChan, disconnecting):
        ...
        status.source = ""
        self.connectionsLock.acquire()
        connectionCopy = dict.copy(self.connections)
        self.connectionsLock.release()

        if inputChan in connectionCopy:
            connection = connectionCopy[inputChan]
    ...
    def sendData(self):
        ...
        for outputChan in {c.outputChan for c in connectionsCopy.values()}:
        ...
        for inputChan, connection in connectionsCopy.items():
